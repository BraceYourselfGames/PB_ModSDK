...
{
    [Serializable]
    public class CombatActionAddStatus : ICombatActionExecutionFunction
    {
        ...
        public void Run (CombatEntity unitCombat, ActionEntity action)
        {
            ...
            if (unitCombat == null || action == null || !action.hasDuration || !action.hasStartTime)
                return;
            ...
            {
                int updateCount = Mathf.RoundToInt (DataHelperStats.GetCachedStatForUnit (durationFullOverrideStat, unitPersistent));
                if (updateCount > 0)
                    durationUpdateOverride = 1f / updateCount;
            }
            
            UnitStatusUtility.AddStatus (unitCombat, key, UnitStatusSource.Action, durationFullOverride, durationUpdateOverride);
            #endif
        }
