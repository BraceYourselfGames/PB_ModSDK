...
public class UITexturePacker
{
    ...
    private static string exceptionTextLast = null;
    ...
    private static Texture2D readableTex;

    public static Texture2D DuplicateTexture2D (Texture2D source)
    {
        ...
        if (readableTex == null)
            readableTex = new Texture2D (source.width, source.height);
        else if (source.width != readableTex.width || source.height != readableTex.height)
        {
			#if UNITY_2021_2_OR_NEWER
			readableTex.Reinitialize (source.width, source.height);
			#else
            readableTex.Resize (source.width, source.height);
			#endif
        }
    ...
    static public Rect[] PackTextures (Texture2D texAtlas, Texture2D[] textures, int width, int height, int padding, int maxSize, bool forceSquare)
    {
        ...
        for (int i = 0; i < textures.Length; i++)
        {
            ...
            for (xPadding = 1; xPadding >= 0; --xPadding)
            {
                for (yPadding = 1; yPadding >= 0; --yPadding)
                {
                    ...
                    {
                        return PackTextures (texAtlas, textures, width * (width <= height ? 2 : 1),
                            height * (height < width ? 2 : 1), padding, maxSize, forceSquare);
                    }
                }
		...
		#if UNITY_2021_2_OR_NEWER
		texAtlas.Reinitialize(width, height);
		#else
        texAtlas.Resize (width, height);
		#endif
        texAtlas.SetPixels (new Color[width * height]);
        ...
        {
            var texPacked = textures[i];
            if (!texPacked) continue;
            ...
            try
            {
                colors = texPacked.GetPixels ();
            }
            catch (Exception e)
            {
                // Debug.LogWarning (e);
                exceptionTextLast = e.ToString ();
                readable = false;
            }
            ...
            {
                countCopied += 1;
                Texture2D texPackedCopy = DuplicateTexture2D (texPacked);
                colors = texPackedCopy.GetPixels ();
                // Debug.LogWarning ($"Copying non-readable texture {texPacked.name} to sample it");
            }

            // Would be used to rotate the texture if need be.
            if (rect.width != texPacked.width + xPadding)
            {
                var newColors = texPacked.GetPixels ();

                for (int x = 0; x < rect.width; x++)
                {
                    for (int y = 0; y < rect.height; y++)
                    {
                        int prevIndex = ((int)rect.height - (y + 1)) + x * (int)texPacked.width;
            ...
            texAtlas.SetPixels ((int)rect.x, (int)rect.y, (int)rect.width - xPadding, (int)rect.height - yPadding, colors);
            rect.x /= width;
            rect.y /= height;
        ...
        texAtlas.Apply ();
        if (countCopied > 0)
            Debug.LogWarning ($"{countCopied} out of {count} textures required copying via temporary RT to access non-readable data");
    ...
    public Rect Insert (int width, int height, FreeRectChoiceHeuristic method)
    {
        ...
        switch (method)
        {
            case FreeRectChoiceHeuristic.RectBestShortSideFit:
                newNode = FindPositionForNewNodeBestShortSideFit (width, height, ref unused0, ref unused1);
                break;
            case FreeRectChoiceHeuristic.RectBottomLeftRule:
                newNode = FindPositionForNewNodeBottomLeft (width, height, ref unused0, ref unused1);
                break;
            case FreeRectChoiceHeuristic.RectContactPointRule:
                newNode = FindPositionForNewNodeContactPoint (width, height, ref unused0);
                break;
            case FreeRectChoiceHeuristic.RectBestLongSideFit:
                newNode = FindPositionForNewNodeBestLongSideFit (width, height, ref unused1, ref unused0);
                break;
            case FreeRectChoiceHeuristic.RectBestAreaFit:
                newNode = FindPositionForNewNodeBestAreaFit (width, height, ref unused0, ref unused1);
                break;
        }
