...
public class AsmdefDebug
{
    ...
    static void AssemblyReloadEventsOnBeforeAssemblyReload() 
    {
        ...
        s_BuildEvents.AppendFormat("- {0:0.00}s \t Compilation total\n", totalCompilationTimeSeconds); 
    ...
    {
        if (!EditorPrefs.HasKey (AssemblyReloadEventsEditorPref))
        {
            Debug.LogWarning ($"EditorPrefs have no key {AssemblyReloadEventsEditorPref}, failed to fetch assembly compile time");
            return;
        }
        
        if (!EditorPrefs.HasKey (AssemblyTotalCompilationTimeEditorPref))
        {
            Debug.LogWarning ($"EditorPrefs have no key {AssemblyTotalCompilationTimeEditorPref}, failed to fetch assembly compile time");
            return;
        }
        ...
        if (long.TryParse(binString, out bin))
        {
            ...
            {
                Debug.Log($"{DateTime.Now.ToShortTimeString ()} | Compilation/reload completed in {totalTimeSeconds:F2}s (detailed breakdown follows):\n{compilationTimes}- {time.TotalSeconds:F2}s \t <b>Assembly reload time</b>");
            } 
        }
    }
}
